...
namespace ModUntitled {
    public partial class Animation {
        public partial class Collection {
            ...
            public Collection(YAML.Collection deserialized, string base_dir) {
                _DeserializedYAMLDoc = deserialized;

                GameObject = new GameObject($"ModTheGungeon Collection '{deserialized.Name}'");

                _Generator = new CollectionGenerator(base_dir, _DeserializedYAMLDoc, GameObject);
                CollectionData = _Generator.ConstructCollection();
            }

            public Collection(Dictionary<string, Texture2D> textures, string text)
                : this(textures, _Deserializer.Deserialize<YAML.Collection>(text)) { }

            public Collection(Dictionary<string, Texture2D> textures, YAML.Collection deserialized) {
                _DeserializedYAMLDoc = deserialized;
                GameObject = new GameObject($"ModTheGungeon Collection '{deserialized.Name}'");

                _Generator = new CollectionGenerator(textures, _DeserializedYAMLDoc, GameObject);
                CollectionData = _Generator.ConstructCollection();
            }
